...
public class Main extends javax.swing.JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5282488416140002600L;
...
//    private final FileNameExtensionFilter filtroDocumentos = new FileNameExtensionFilter("Documentos de Oficina", "pdf", "p7m", "docx", "xlsx", "pptx", "odt", "ods", "odp", "xml");
		...
		jtblFirmarDocumentos.addKeyListener(new java.awt.event.KeyAdapter() {
			...
			public void keyPressed(java.awt.event.KeyEvent evt) {
				if (evt.getKeyChar() == java.awt.event.KeyEvent.VK_ENTER) {
					String ruta = (String) tableModelDocumentos.getValueAt(
							jtblFirmarDocumentos.convertRowIndexToModel(jtblFirmarDocumentos.getSelectedRow()), 0);
	...
	private void validacionPdf() throws DocumentoException, RazonFirma {
		...
		if (razonFirma != null && (razonFirma.length() > 70)) {
			throw new RazonFirma(MessageFormat.format(messages.getProperty("mensaje.error.razonfirma_limite"),
					Integer.parseInt(messages.getProperty("limite.razon_firma"))));
		}
		if (point == null) {
			throw new DocumentoException(messages.getProperty("mensaje.error.documento_sin_firma"));
		}
	...
	private void validacionPreFirmar() throws DocumentoNoExistenteException, TokenNoConectadoException,
			DocumentoNoPermitidoException, CertificadoInvalidoException, DocumentoException, RazonFirma {
		// Revisamos si existe el documento a firmar
		// TODO no hacer un return directamente, se podria validarCertificado todos los
		// parametros e ir aumentando los errores
		if (!documento.exists()) {
			throw new DocumentoNoExistenteException(MessageFormat
					.format(messages.getProperty("mensaje.error.documento_inexistente"), documento.getPath()));
		}
	...
	private void verificarDocumento() throws Exception {
		...
		certificados.stream().map((certificado) -> {
			...
			String validarFirma = Utils.validarFirma(certificado.getValidFrom(), certificado.getValidTo(),
					certificado.getGenerated(), certificado.getRevocated());
		...
		if (certificados != null || !certificados.isEmpty()) {
			if (certificados.get(0).getDocVerify() != null && !certificados.get(0).getDocVerify()) {
				JOptionPane.showMessageDialog(null, messages.getProperty("mensaje.error.documento_modificado"), "Error",
	...
	private void tipoDeDocumentPermitido(File documento) throws DocumentoNoPermitidoException {
		...
		if (!extensionesDocumentos.stream().anyMatch((extension) -> (extension.equals(extDocumento)))) {
			throw new DocumentoNoPermitidoException(
					MessageFormat.format(messages.getProperty("mensaje.error.extension_no_permitida"), extDocumento));
		}
	}
	...
	private KeyStore getKeyStore() throws TokenNoEncontradoException {
		...
		try {
			if (jrbFirmarToken.isSelected()) {
				...
				if (ks == null) {
					throw new TokenNoEncontradoException(messages.getProperty("mensaje.error.token_contrasena_invalida")
							+ " o " + messages.getProperty("mensaje.error.token_no_encontrado"));
				}
			}
			if (jrbFirmarArchivo.isSelected()) {
				...
				if (llave.exists() == true) {
					...
					javax.swing.JOptionPane.showMessageDialog(null,
							"No se encontró el certificado digital " + llave.getPath(), "Error",
							javax.swing.JOptionPane.ERROR_MESSAGE);
				}
			}
			...
			if (e.getMessage().equals("java.io.IOException: keystore password was incorrect")) {
				JOptionPane.showMessageDialog(this, messages.getProperty("mensaje.error.clave_incorrecta"), "Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, messages.getProperty("mensaje.error.certificado_formato_invalido"),
	...
	private void firmarDocumento(List<String> rutaDocumentos) {
		...
		try {
				...
				if (x509CertificateUtils.validarX509Certificate(x509Certificate)) {
					if (x509Certificate != null && alias != null) {
						...
						new Thread(() -> {
							...
							ProgressMonitor pm = new ProgressMonitor(this, "Firmando", "Task starting", 1,
									rutaDocumentos.size());
							int i = 0;
							for (String rutaDocumento : rutaDocumentos) {
								try {
									...
									byte[] docSigned = null;
									docSigned = FirmaDigital.firmar(ks, alias, documento, password, point, pagina,
											razonFirma, tipoFirma);
									if (docSigned != null) {
										// Información del documento firmado y firmante
										String nombreDocFirmado = FileUtils.crearNombreFirmado(documento,
									...
									// Reseteamos el campo de archivo firmado y las tablas de informacion para que
									// no hay confusión
									...
									if (mensaje != null) {
										...
										if (mensaje.contains("IllegalStateException")
												&& mensaje.contains("Content_Types")) {
											mensaje = messages.getProperty("mensaje.error.documento_corrupto");
										}
										if (mensaje.contains(
												"Las firmas XAdES Enveloped solo pueden realizarse sobre datos XML")) {
			...
			jpfValidarClave.setText("");
			jplCertificadoValidar.setEnabled(true);
		} catch (TokenNoEncontradoException | CertificadoInvalidoException | EntidadCertificadoraNoValidaException
				| HoraServidorException | RubricaException | HeadlessException | IOException | InvalidKeyException
				| DocumentoNoExistenteException | TokenNoConectadoException | DocumentoNoPermitidoException
				| DocumentoException | RazonFirma ex) {
			enableControles(true);
			// Reseteamos el campo de archivo firmado y las tablas de informacion para que
			// no hay confusión
			resetFirmarDocumento();
	...
	private void setearInfoValidacionCertificado(X509Certificate cert) throws CertificadoInvalidoException,
			InvalidKeyException, io.rubrica.exceptions.EntidadCertificadoraNoValidaException {
		if (cert != null) {
			...
			if (datosUsuario == null
					&& (jpfValidarClave.getPassword() == null || jpfValidarClave.getPassword().length == 0)) {
				throw new CertificadoInvalidoException(
						messages.getProperty("mensaje.error.extraer_datos_certificados"));
			}
			...
			data[0] = MessageFormat.format(messages.getProperty("tabla.certificado.emitido_por"),
					CertEcUtils.getNombreCA(cert));
	...
	private void agregarValidezCertificado(X509CertificateUtils x509CertificateUtils) {
		...
		if (x509CertificateUtils.isDesconocido()) {
			data[0] = MessageFormat.format(messages.getProperty("tabla.certificado.emitido_por"),
					messages.getProperty("tabla.certificado.desconocido"));
			tableModel.setValueAt(data[0], 0, 0);
		}
		data[0] = messages.getProperty("tabla.certificado.caducado") + " "
				+ (x509CertificateUtils.isCaducado() ? messages.getProperty("mensaje.si")
						: messages.getProperty("mensaje.no"));
		tableModel.addRow(data);
		data[0] = messages.getProperty("tabla.certificado.revocado")
				+ (x509CertificateUtils.getRevocado() != null ? x509CertificateUtils.getRevocado()
						: messages.getProperty("mensaje.no"));
	...
	public void actualizar() {
		...
		btnSi.addActionListener((java.awt.event.ActionEvent evt) -> {
			...
			try {
				...
				JOptionPane.showOptionDialog(getParent(), messages.getProperty("mensaje.actualizado"), "Mensaje",
						JOptionPane.OK_OPTION, JOptionPane.INFORMATION_MESSAGE, null, optionsAceptar, btnAceptar);

				System.exit(0);
			} catch (IllegalArgumentException e) {
				JOptionPane.showOptionDialog(getParent(),
						messages.getProperty("mensaje.error.actualizar_administracion"), "Error", JOptionPane.OK_OPTION,
						JOptionPane.ERROR_MESSAGE, null, optionsAceptar, btnAceptar);
			} catch (IOException e) {
				LOGGER.log(Level.SEVERE, "Error al actualizar:", e);
				JOptionPane.showOptionDialog(getParent(),
						messages.getProperty("mensaje.error.actualizar") + ": " + e.getMessage(), "Error",
		...
		JOptionPane.showOptionDialog(getParent(), version + messages.getProperty("mensaje.desea_actualizar"),
				messages.getProperty("mensaje.confirmar"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE,
	...
	private List<String> existeDocumentos() {
		...
		for (int i = 0; i < tableModelDocumentos.getRowCount(); i++) {
			...
			if (rutaDocumento != null) {
				...
				javax.swing.JOptionPane.showMessageDialog(null, MessageFormat
						.format(messages.getProperty("mensaje.error.documento_inexistente"), rutaDocumento), "Error",
						javax.swing.JOptionPane.ERROR_MESSAGE);
			}
		}
		return documentos;
	...
	private void initComponents() {
		...
		miPredeterminar = new javax.swing.JMenuItem();
		...
		jplCertificadoFirmarLayout.setHorizontalGroup(jplCertificadoFirmarLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplCertificadoFirmarLayout.createSequentialGroup().addContainerGap()
						.addGroup(jplCertificadoFirmarLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addGroup(jplCertificadoFirmarLayout.createSequentialGroup()
										...
										.addGroup(jplCertificadoFirmarLayout
												...
												.addGroup(jplCertificadoFirmarLayout.createSequentialGroup()
														...
														.addComponent(jrbFirmarToken).addGap(0, 523, Short.MAX_VALUE)))
										.addContainerGap())
								.addGroup(jplCertificadoFirmarLayout.createSequentialGroup().addComponent(jblClave)
										.addGap(20, 20, 20).addComponent(jpfFirmarClave).addGap(102, 102, 102)))));
		jplCertificadoFirmarLayout.setVerticalGroup(
				jplCertificadoFirmarLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGroup(
						javax.swing.GroupLayout.Alignment.TRAILING,
						jplCertificadoFirmarLayout.createSequentialGroup().addContainerGap()
								.addGroup(jplCertificadoFirmarLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(jrbFirmarArchivo).addComponent(jrbFirmarToken)
										.addComponent(jblCertificadoEnFimador))
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addGroup(jplCertificadoFirmarLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(jblCertificadoFirmar)
										.addComponent(jtxtFirmarRuta, javax.swing.GroupLayout.PREFERRED_SIZE, 33,
												javax.swing.GroupLayout.PREFERRED_SIZE)
										.addComponent(jbtnFirmarExaminarCertificado))
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addGroup(jplCertificadoFirmarLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(jpfFirmarClave, javax.swing.GroupLayout.PREFERRED_SIZE, 35,
												javax.swing.GroupLayout.PREFERRED_SIZE)
		...
		}, new String[] { "Doble clic sobre el documento para visualizar" }) {
		...
		jplDocumentosLayout.setHorizontalGroup(jplDocumentosLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplDocumentosLayout.createSequentialGroup().addContainerGap()
						.addGroup(jplDocumentosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addComponent(jlblDocumentosFirmar, javax.swing.GroupLayout.Alignment.CENTER,
										javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addComponent(jScrollPaneFirmarDocumentos, javax.swing.GroupLayout.Alignment.CENTER)
								.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
										jplDocumentosLayout.createSequentialGroup().addComponent(jchkBoxFirmaInvisible)
												.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED,
						...
						.addContainerGap()));
		jplDocumentosLayout.setVerticalGroup(jplDocumentosLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplDocumentosLayout.createSequentialGroup().addContainerGap()
						.addComponent(jlblDocumentosFirmar, javax.swing.GroupLayout.PREFERRED_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(
								jScrollPaneFirmarDocumentos, javax.swing.GroupLayout.DEFAULT_SIZE, 159, Short.MAX_VALUE)
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addGroup(jplDocumentosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jbtnFirmarExaminarDocumentos).addComponent(jbtnFirmarEliminarDocumentos)
		...
		jplFirmarDocumentoLayout.setHorizontalGroup(jplFirmarDocumentoLayout
				...
				.addGroup(jplFirmarDocumentoLayout.createSequentialGroup().addContainerGap()
						.addGroup(jplFirmarDocumentoLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
										jplFirmarDocumentoLayout.createSequentialGroup()
		...
		jplDocumentoVerificarLayout.setHorizontalGroup(
				jplDocumentoVerificarLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplDocumentoVerificarLayout.createSequentialGroup().addContainerGap()
								.addComponent(jlbArchivoFirmadoVerficar)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
								.addComponent(jtxtVerificarRuta).addGap(18, 18, 18).addComponent(jbtnVerificarExaminar)
								.addContainerGap()));
		jplDocumentoVerificarLayout.setVerticalGroup(
				jplDocumentoVerificarLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplDocumentoVerificarLayout.createSequentialGroup()
								.addGroup(jplDocumentoVerificarLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(jtxtVerificarRuta, javax.swing.GroupLayout.PREFERRED_SIZE, 34,
												javax.swing.GroupLayout.PREFERRED_SIZE)
										.addComponent(jbtnVerificarExaminar).addComponent(jlbArchivoFirmadoVerficar))
								.addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
		...
		}, new String[] { "Cédula", "Nombres", "Razón", "Entidad Certificadora", "Fecha Firmado", "Firma" }) {
		...
		jplVerificacionLayout.setHorizontalGroup(jplVerificacionLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplVerificacionLayout.createSequentialGroup().addContainerGap()
						.addGroup(jplVerificacionLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.CENTER)
								.addComponent(jlblResultadoVerificacion, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addComponent(jScrollPaneVerificarDatosFirmante, javax.swing.GroupLayout.DEFAULT_SIZE,
										753, Short.MAX_VALUE))
						.addContainerGap()));
		jplVerificacionLayout
				.setVerticalGroup(jplVerificacionLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplVerificacionLayout.createSequentialGroup().addContainerGap()
								.addComponent(jlblResultadoVerificacion, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
		...
		jplVerificarDocumentoLayout.setHorizontalGroup(
				jplVerificarDocumentoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplVerificarDocumentoLayout.createSequentialGroup().addContainerGap()
								.addGroup(jplVerificarDocumentoLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
										.addComponent(jplVerificacion, javax.swing.GroupLayout.DEFAULT_SIZE,
												javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
										.addComponent(jplDocumentoVerificar, javax.swing.GroupLayout.DEFAULT_SIZE, 769,
												Short.MAX_VALUE))
								.addGap(8, 8, 8))
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
		...
		jplVerificarDocumentoLayout.setVerticalGroup(
				jplVerificarDocumentoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplVerificarDocumentoLayout.createSequentialGroup().addContainerGap()
								.addComponent(jplDocumentoVerificar, javax.swing.GroupLayout.PREFERRED_SIZE, 44,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jplVerificacion, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
		...
		jplCertificadoValidarLayout.setHorizontalGroup(jplCertificadoValidarLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplCertificadoValidarLayout.createSequentialGroup().addContainerGap()
						.addGroup(jplCertificadoValidarLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addComponent(jlbCertificadoValidar).addComponent(jlbCertificadoVldCert)
						...
						.addGroup(jplCertificadoValidarLayout
								...
								.addGroup(jplCertificadoValidarLayout.createSequentialGroup()
										...
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
										.addComponent(jrbValidarToken).addGap(0, 0, Short.MAX_VALUE))
								.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
										jplCertificadoValidarLayout.createSequentialGroup()
												.addGroup(jplCertificadoValidarLayout
														.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
														.addComponent(jpfValidarClave,
																javax.swing.GroupLayout.Alignment.LEADING)
														.addComponent(jtxtValidarRuta))
												.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
												.addComponent(jbtnValidarExaminar)))
						.addContainerGap()));
		jplCertificadoValidarLayout.setVerticalGroup(jplCertificadoValidarLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jplCertificadoValidarLayout
						.createSequentialGroup().addContainerGap()
						.addGroup(jplCertificadoValidarLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jrbValidarArchivo).addComponent(jrbValidarToken)
								.addComponent(jlbCertificadoValidar))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addGroup(jplCertificadoValidarLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jlbCertificadoVldCert)
								.addComponent(jtxtValidarRuta, javax.swing.GroupLayout.PREFERRED_SIZE, 34,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addComponent(jbtnValidarExaminar))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED,
								javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						.addGroup(jplCertificadoValidarLayout
								.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jpfValidarClave, javax.swing.GroupLayout.PREFERRED_SIZE, 36,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addComponent(jlbCertificadoValidarCert))
						.addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
		...
		}, new String[] { "Datos del Certificado" }) {
			boolean[] canEdit = new boolean[] { false };
		...
		jplValidacionLayout.setHorizontalGroup(jplValidacionLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jplValidacionLayout.createSequentialGroup().addContainerGap().addGroup(jplValidacionLayout
						.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplValidacionLayout.createSequentialGroup().addGap(0, 0, Short.MAX_VALUE)
								.addComponent(jlblResultadoValidacion, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addGap(0, 0, Short.MAX_VALUE))
						.addComponent(jScrollPaneValidarDatosCertificados, javax.swing.GroupLayout.DEFAULT_SIZE, 753,
								Short.MAX_VALUE))
						.addContainerGap()));
		jplValidacionLayout
				.setVerticalGroup(jplValidacionLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplValidacionLayout.createSequentialGroup().addContainerGap()
								.addComponent(jlblResultadoValidacion, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jScrollPaneValidarDatosCertificados, javax.swing.GroupLayout.DEFAULT_SIZE,
										190, Short.MAX_VALUE)
								.addContainerGap()));
				...
				jplValidarCertificadoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jplValidarCertificadoLayout.createSequentialGroup().addContainerGap()
								.addGroup(jplValidarCertificadoLayout
										.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
										.addComponent(jplCertificadoValidar, javax.swing.GroupLayout.Alignment.LEADING,
												javax.swing.GroupLayout.DEFAULT_SIZE,
												javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
										.addComponent(jplValidacion, javax.swing.GroupLayout.Alignment.LEADING,
												javax.swing.GroupLayout.DEFAULT_SIZE,
												javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
								.addGap(8, 8, 8))
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
		...
		jmConfiguracion.setMnemonic('a');
		jmConfiguracion.setText("Archivo");
		...
		jmiPanelConfiguracion.setMnemonic('c');
		jmiPanelConfiguracion.setText("Configuración");
		...
		jmConfiguracion.add(new JSeparator());
		...
		layout.setVerticalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(layout.createSequentialGroup().addComponent(jScrollPane1).addGap(0, 0, 0)));
	...
	private void jmiAcercaActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jmiAcercaActionPerformed
		...
		JOptionPane.showOptionDialog(getParent(), jplAcercaDe, "Acerca de FirmaEC", JOptionPane.OK_OPTION,
		...
		try {
			...
			if (update.update() == null) {
				JOptionPane.showMessageDialog(rootPane,
						"Se encuentra en su última versión, no es necesario actualizar");
			} else {
				actualizar();
			}
		} catch (IOException ex) {
	...
	private void jmiPanelConfiguracionActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jmiPanelConfiguracionActionPerformed
		...
		JOptionPane.showOptionDialog(null, jPanelConfiguracion, "Panel de Configuración", JOptionPane.OK_OPTION,
				JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
	...
	private void jbtnValidarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jbtnValidarActionPerformed
		if (jrbValidarToken.isSelected() || jrbValidarArchivo.isSelected()) {
			KeyStoreProvider ksp;
			setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
			KeyStore ks = null;
			try {
				// Si es linux y quiere validarCertificado token debe tener por lo menos una
				// clave
				if ((OS.contains("linux") || OS.contains("mac")) && this.jrbValidarToken.isSelected()
						&& jpfValidarClave.getPassword().length == 0) {
				...
				if (this.jrbValidarToken.isSelected()) {
					...
					if (ks == null) {
						throw new TokenNoEncontradoException(
								messages.getProperty("mensaje.error.token_contrasena_invalida") + " o "
										+ messages.getProperty("mensaje.error.token_no_encontrado"));
					}
				}
				...
				if (e.getMessage().equals("java.io.IOException: keystore password was incorrect")) {
					...
					JOptionPane.showMessageDialog(this,
							messages.getProperty("mensaje.error.certificado_formato_invalido"), "Error",
							JOptionPane.ERROR_MESSAGE);
				}
				jpfValidarClave.setText("");
				jplCertificadoValidar.setEnabled(true);
			} catch (TokenNoEncontradoException | CertificadoInvalidoException | EntidadCertificadoraNoValidaException
					| HoraServidorException | RubricaException | HeadlessException | IOException
			...
			javax.swing.JOptionPane.showMessageDialog(this, "Seleccione tipo de certificado", "Error",
					javax.swing.JOptionPane.ERROR_MESSAGE);
		}
		...
		if (!jtxtVerificarRuta.getText().isEmpty()) {
			...
			if (documento.exists() == true) {
				...
				try {
					...
					Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
					JOptionPane.showMessageDialog(this,
							messages.getProperty("mensaje.error.archivo_no_encontrado") + ": " + ex.getMessage(),
							"Error", JOptionPane.ERROR_MESSAGE);
					jplDocumentoVerificar.setEnabled(true);
				...
				javax.swing.JOptionPane.showMessageDialog(null, "No se encontró el documento: " + documento.getPath(),
						"Error", javax.swing.JOptionPane.ERROR_MESSAGE);
			}
		} else {
			javax.swing.JOptionPane.showMessageDialog(this, "Seleccione documento", "Error",
					javax.swing.JOptionPane.ERROR_MESSAGE);
		}
	...
	private void jbtnFirmarEliminarDocumentosActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jbtnFirmarEliminarDocumentosActionPerformed
		if (jtblFirmarDocumentos.getSelectedRow() >= 0) {
			tableModelDocumentos
					.removeRow(jtblFirmarDocumentos.convertRowIndexToModel(jtblFirmarDocumentos.getSelectedRow()));
		} else {
			JOptionPane.showMessageDialog(this, messages.getProperty("mensaje.error.seleccion_documento"), "Error",
					javax.swing.JOptionPane.ERROR_MESSAGE);
		}
	...
	private void previewPdf() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this,
					messages.getProperty("mensaje.error.documento_problemas") + "\n" + documento, "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public static void main(String args[]) {
		...
		if (args != null && args.length > 0 && ("--update".equals(args[0]) || "--updatedes".equals(args[0]))) {
			...
			LOGGER.log(Level.INFO, "Firmador: {0} JRE: {1} Sistema Operativo: {2}",
					new Object[] { version, OsUtils.getJavaVersion(), OsUtils.getOs() });
		}
	}
	...
	private static void update(String version) {
		...
		if (!version.contains("BETA")) {
			try {
				...
				if (mensaje != null) {
					SwingLink link = new SwingLink(mensaje, "FirmaEC",
							PropertiesUtils.getMessages().getProperty("url_ayuda_online"));
					JOptionPane.showMessageDialog(null, link);
				}
			} catch (IOException ex) {
				LOGGER.log(Level.SEVERE, "No se puede comprobar actualizaciones", ex);
				JOptionPane.showMessageDialog(null, "No se puede comprobar actualizaciones", "Error",
						javax.swing.JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private javax.swing.JMenuItem miPredeterminar;
